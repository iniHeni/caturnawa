              ...
              <tbody>
                  @foreach ($data as $item)
                      <tr>
                          ...
                          <td>1.@lang('messages.lktif1')</td>
                          <td>
                              @if (is_array(explode(', ', $item->scorepemaparanmateri)))
                                  @foreach (explode(', ', $item->scorepemaparanmateri) as $score)
                                      {{ $score }}<br>
                                  @endforeach
                              @endif
                          </td>
                          <td> @if (is_array(explode(', ', $item->materi)))
                            @foreach (explode(', ', $item->materi) as $score)
                                {{ $score }}<br>
                            @endforeach
                        @endif</td> 
                        <td rowspan="3">
                      ...
                      <tr>
                          <td>2.@lang('messages.lktif2')</td>
                          <td>
                              @if (is_array(explode(', ', $item->scorepertanyaandanjawaban)))
                                  @foreach (explode(', ', $item->scorepertanyaandanjawaban) as $score)
                                      {{ $score }}<br>
                                  @endforeach
                              @endif
                          </td>
                          <td> @if (is_array(explode(', ', $item->pertanyaandanjawaban)))
                            @foreach (explode(', ', $item->pertanyaandanjawaban) as $score)
                                {{ $score }}<br>
                            @endforeach
                        @endif</td>
                      </tr>
                      <tr>
                          <td>3.@lang('messages.lktif3')</td>
                          <td>
                              @if (is_array(explode(', ', $item->scoreaspekkesesuaian)))
                                  @foreach (explode(', ', $item->scoreaspekkesesuaian) as $score)
                                      {{ $score }}<br>
                                  @endforeach
                              @endif
                          </td>
                          <td>@if (is_array(explode(', ', $item->kesesuaian)))
                            @foreach (explode(', ', $item->kesesuaian) as $score)
                                {{ $score }}<br>
                            @endforeach
                        @endif</td>
                      </tr>
