    ...
    <section id="skor">
        <div class="container" style="display: flex; justify-content: center;height:70rem">
            <div style="width: 100%;">
                ...
                <p><a class="add" href="{{ route('edc.pesertaday2f') }}" style="color: white" onclick="
                    ...
                    @else
                        window.location.href = '{{ route('edc.pesertaday2f') }}';
                    @endif
                            ...
                            <td>
                                <a href="#" onclick="
                                    ...
                                    @if (!$today->greaterThanOrEqualTo($tambah))
                                        Swal.fire('Ditutup', 'Pengeditan Nilai sudah ditutup,jika ada kendala silahkan menghubungi pihak terkait', 'info');
                                    @else
                                        window.location.href = '{{ route('edc.editedc5', $data->id) }}';
                                    @endif
                                ">Edit</a>
                                <form action="{{ route('edc.hapusedc5', $data->id) }}" method="POST" id="delete-form-{{ $data->id }}">
                                    @csrf
