...
<div id="data-container">
    <section id="skor">
        <div class="container" style="display: flex; justify-content: center;height:70rem">
            <div style="width: 100%;">
                <h1 class="welcome">Semifinal</h1>
                <p><a class="add" href="{{ route('kdbi.pesertasf') }}" style="color: white">Tambah Penilaian</a></p>
                <div class="table-responsive" style="max-height: 1000px; overflow-x: auto; overflow-y: auto; position: static;">
                    @foreach ($groupedByRondeAndSesi as $ronde => $dataRonde)
                    @foreach ($dataRonde as $room => $dataa)
                    <div style="display: flex; align-items: center; justify-content: center; margin-bottom: 10px;">
                        <h2 style="color: white; text-align:center">Ronde {{ $ronde }} |</h2>
                        <h3 style="color: white; text-align:center; margin-left: 10px;">Room {{ $room }}</h3></div>
                            ...
                            <tr>
                                <td rowspan="20">{{ $dataa->first()->juri }}</td> 
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $item->vp}}</td>
                                            <td>
                                                <a href="{{ route('kdbi.editkdbi3', $item->id) }}">Edit</a>
                                                <form action="{{ route('kdbi.hapuskdbi3', $item->id) }}" method="POST" id="delete-form-{{ $item->id }}">
                                                    @csrf 
