    ...
    <section id="skor">
        <div class="container" style="display: flex; justify-content: center;height:70rem">
            <div style="width: 100%;">
                ...
                <p><a class="add" href="{{ route('kdbi.pesertaday2') }}" style="color: white">Tambah Penilaian</a></p>
    ...
    @foreach ($sesiChunks as $sesi => $dataSesi)
                    <table class="table table-bordered table-striped" style="min-width: 650px; margin-bottom: 5rem; border-collapse: collapse;">
                        ...
                        <tr>
                            ...
                            <td>{{ $data->vp}}</td>
                            <td>
                                <a href="{{ route('kdbi.editkdbi2', $data->id) }}">Edit</a>
                                <form action="{{ route('kdbi.hapuskdbi2', $data->id) }}" method="POST" id="delete-form-{{ $data->id }}">
                                    @csrf 
